...
#import "MFImageScrollView.h"
#import "MFWebImageManager.h"

@interface MFImageScrollView () <UIScrollViewDelegate>

@property (nonatomic,copy) NSArray *imageData;

@end

@implementation MFImageScrollView{
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[MFImageScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
    ...
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[MFWebImageManager shareManager] webImageCache] valueForKey:_imageData[index]];
...
- (void)getImage {
    ...
    for (NSString *urlSting in _imageData) {
        [[MFWebImageManager shareManager] downloadImageWithUrlString:urlSting];
    }
