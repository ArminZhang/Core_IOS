...
- (void)addTopBorderWithColor:(NSString *)color andWidth:(CGFloat) borderWidth {
    ...
    layer.topWidth = borderWidth;
    layer.topColor = [color toUIColor:1.0];
...
- (void)addBottomBorderWithColor:(NSString *)color andWidth:(CGFloat) borderWidth {
    BorderLayer *layer = [BorderLayer layer];
    layer.frame = self.layer.bounds;
    layer.bottomWidth = borderWidth;
    layer.bottomColor = [color toUIColor:1.0];
    [layer setNeedsDisplay];
    [self.layer addSublayer:layer];
}
...
- (void)addLeftBorderWithColor:(NSString *)color andWidth:(CGFloat) borderWidth {
    BorderLayer *layer = [BorderLayer layer];
    layer.frame = self.layer.bounds;
    layer.leftWidth = borderWidth;
    layer.leftColor = [color toUIColor:1.0];
    [layer setNeedsDisplay];
    [self.layer addSublayer:layer];
}
...
- (void)addRightBorderWithColor:(NSString *)color andWidth:(CGFloat) borderWidth {
    BorderLayer *layer = [BorderLayer layer];
    layer.frame = self.layer.bounds;
    layer.rightWidth = borderWidth;
    layer.rightColor = [color toUIColor:1.0];
    [layer setNeedsDisplay];
    [self.layer addSublayer:layer];
}
...
-(void)drawInContext:(CGContextRef)ctx
{
    ...
    if (_leftWidth > 0) {
        ...
        CGContextSetStrokeColorWithColor(ctx, _leftColor.CGColor);
    ...
    if (_topWidth > 0) {
        ...
        CGContextClosePath(ctx);
        CGContextSetStrokeColorWithColor(ctx, _topColor.CGColor);
        CGContextDrawPath(ctx, kCGPathFillStroke);
    }
    ...
    if (_rightWidth > 0) {
        ...
        CGContextClosePath(ctx);
        CGContextSetStrokeColorWithColor(ctx, _rightColor.CGColor);
        CGContextDrawPath(ctx, kCGPathFillStroke);
    }
    ...
    if (_bottomWidth > 0) {
        ...
        CGContextAddLineToPoint (ctx, x + width, y + height);
        CGContextClosePath(ctx);
        CGContextSetStrokeColorWithColor(ctx, _bottomColor.CGColor);
        CGContextDrawPath(ctx, kCGPathFillStroke);
    }
}
